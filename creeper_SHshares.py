...
print("Hello! this is a creeper program")  # greetint
...
def single_share(name):
    ...
    company_url = "https://stock.xueqiu.com/v5/stock/f10/cn/company.json?"
    url = "https://stock.xueqiu.com/v5/stock/chart/kline.json?"
    company_parameter = {
        "symbol": name,
    }
    response = requests.get(company_url, headers=header,
                            params=company_parameter)
    begin = response.json()["data"]["company"]["listed_date"]
    if begin is None:
        return None
    parameter = {
        "symbol": name,
...
for number in range(0, length):  # for every stock
    ...
    if J == None:
        ...
        print(tm)
        continue
    with open("SH{}.share_data".format(base_number+number), "w") as fo:
        print("seiko")
        fo.write(str(J["data"]["column"])+"\n")
